...
class ErrorHandler {
    ...
    error(error: HttpException, request: Request, response: Response, next: NextFunction): void {
        ...
        // const cloudEventsHelper = new CloudEventsHandler(['kweli_error']);
        // const env = process.env.NODE_ENV ?? '';
        // cloudEventsHelper.addData('kweli_error', { slack: { text: `:new: *KAPI ERROR*\n\n:rocket: Environment - ${env}\n\n:1234: Code - ${code}\n\n:x: Detail - ${detail}\n\n:triangular_flag_on_post: Method - ${method}\n\n:motorway: Path - ${path}`, channel: 'system_alerts' } });
